<template>
  <div class="space-y-6">
    ...
    <!-- Cabecera con contador de no leídas y botón de marcar todas -->
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-800">Notificaciones</h1>
        <!-- Muestra el número de notificaciones sin leer -->
        <p class="text-sm text-gray-400 mt-1">
          {{ noLeidas }} notificación{{ noLeidas !== 1 ? 'es' : '' }} sin leer
        </p>
      </div>
      <!-- Botón visible solo si hay notificaciones sin leer -->
      <button
        v-if="noLeidas > 0"
        @click="leerTodas"
    ...
    <!-- Indicador de carga mientras se obtienen las notificaciones -->
    ...
    <div v-else class="space-y-3">
      <div
        ...
        <!-- Las notificaciones leídas se muestran con opacidad reducida -->
        :class="notificacion.leida ? 'opacity-60' : ''"
      >
        <!-- Icono con color diferenciado según el tipo de notificación -->
        <div
        ...
        <!-- Contenido: título, fecha, cuerpo y badges -->
        <div class="flex-1 min-w-0">
          ...
          <!-- Cuerpo de la notificación — opcional -->
          ...
          <div class="flex items-center gap-3 mt-2">
            ...
            <!-- Indicador de notificación nueva — solo si no está leída -->
    ...
    <!-- Mensaje cuando no hay notificaciones para el filtro seleccionado -->
    <div
...
/** Lista reactiva de notificaciones del usuario autenticado */
...
/** Filtro activo por estado de lectura */
const filtroActivo = ref('todas')

/** Opciones de filtro disponibles por estado de lectura */
const filtros = [
 ...
 * Se recalcula automáticamente al marcar notificaciones como leídas.
 */
...
/**
 * Lista de notificaciones filtrada según el estado de lectura seleccionado.
 * - 'no_leidas': solo las no leídas
 * - 'leidas': solo las leídas
...
// Métodos del composable WebSocket para escuchar notificaciones en tiempo real
const { alEsperaDisponible, quitarListener } = useSocket()

/**
 * Marca una notificación individual como leída actualizando el estado local
...
async function leerUna(notificacion: Notificacion) {
  try {
    ...
    // Actualización local inmediata para evitar recargar toda la lista
...
/**
 * Marca todas las notificaciones del usuario como leídas en una única petición
 * y actualiza el estado local de forma inmediata.
 */
async function leerTodas() {
  try {
    await marcarTodasLeidas()
    // Marcar todas como leídas en el estado local sin recargar
...
/** Devuelve el gradiente de color del icono según el tipo de notificación */
...
/** Devuelve la clase de icono PrimeIcons según el tipo de notificación */
...
/** Devuelve las clases CSS del badge según el tipo de notificación */
...
/** Devuelve la etiqueta legible del tipo de notificación para el badge */
...
/**
 * Formatea una fecha ISO a formato legible corto en español.
 * Ejemplo: "5 may, 09:00"
...
/**
 * Manejador del evento WebSocket 'espera:disponible'.
 * Recarga la lista completa de notificaciones al recibir un aviso en tiempo real,
 * asegurando que la nueva notificación aparezca inmediatamente en la vista.
 * @param _datos Datos del evento (espacioId y fecha) — no usados directamente
 */
...
/**
 * Al montar el componente, carga las notificaciones del usuario
 * y registra el listener WebSocket para recibir nuevas notificaciones
 * de lista de espera en tiempo real.
 */
onMounted(async () => {
  try {
